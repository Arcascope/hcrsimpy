...
class SinglePopModel:
    ...
    def __init__(self, LightFun, Mel):
	...
	LightPhase=self.sigma*Bhat-self.A1*Bhat*0.5*(pow(R,3.0)+1.0/R)*sp.sin(Psi+self.BetaL)-self.A2*Bhat*0.5*(1.0+pow(R,8.0))*sp.sin(2.0*Psi+self.BetaL2);
        melAmp=self.A1m*0.5*self.Mel(t)*(1.0-R**4)*sp.cos(Psi+self.BetaM1)
        melPhase=self.Mel(t)*(self.sigmam-self.A1m*0.5*(R**3+1.0/R)*sp.sin(Psi+self.BetaM1))

        dydt=np.zeros(3)
        
	dydt[0]=-1.0*self.gamma*R+self.K*sp.cos(self.Beta1)/2.0*R*(1.0-pow(R,4.0))+LightAmp+melAmp;
	dydt[1]=self.w0+self.K/2.0*sp.sin(self.Beta1)*(1+pow(R,4.0))+LightPhase+melPhase;
