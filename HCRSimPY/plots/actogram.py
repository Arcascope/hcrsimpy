...
class actogram(object):
    ...
    def __init__(self, ax, tsdf, threshold=10.0):
        """Create an actogram object from time series data given as a pandas dataframe with the columns Time, Phase,
        Light_Level, it can have more columns but must have those at least
        Time should be measured in hours and Phase should be unwrapped phase in radians, light level should be given as
        lux
        """
        self.tsdf = tsdf
    ...
    def getRectangles(self, timeon, timeoff, colorIn='yellow'):
        ...
        r1 = plt.Rectangle((bottom_x, bottom_y), timeoff -
                           timeon, 1, fc=colorIn, alpha=0.5)
        r2 = plt.Rectangle((bottom_x + 24.0, bottom_y),
                           timeoff - timeon, 1, fc=colorIn, alpha=0.5)
        return ((r1, r2))
    ...
    def addLightSchedule(self):
        ...
        for i in range(1, len(lightdata)):
            ...
            if (lightdata[i] < self.threshold and lightsOn):
                self.addRect(lightStart, timedata[i - 1])
                lightsOn = False
                darkOn = True
                darkStart = timedata[i]
            if not(lightsOn) and lightdata[i] >= self.threshold:
                lightsOn = True
    ...
    def addCircadianPhases(self, tsdf2=None, col='blue'):
        """
        This method can be used to add a set of circadian phases onto an axis. You should pass in a time series data
        frame. It is assumed the light data will be identical and we
        ...
        dlmo_func = sp.interpolate.interp1d(
            np.array(
                tsdf2['Phase']), np.array(
                tsdf2['Time']), bounds_error=False)

        real_days = self.tsdf['Time'].iloc[-1] / 24.0

        if (tsdf2.Phase.iloc[0] < 1.309):
            # all the dlmo phases using 1.309 as the cicadian phase of the DLMO
            dlmo_phases = np.arange(1.309, real_days * 2.0 * sp.pi, 2 * sp.pi)
            dlmo_times = np.array(
                list(([fmod(x, 24.0) for x in list(map(dlmo_func, dlmo_phases))])))
            dlmo_times = dlmo_times[np.isfinite(dlmo_times)]
            dayYvalsDLMO = self.num_days - \
                np.arange(0.5, len(dlmo_times) + 0.5, 1.0)
        else:
            # all the dlmo phases using 1.309 as the cicadian phase of the DLMO
            dlmo_phases = np.arange(
                1.309 + 2 * sp.pi,
                real_days * 2.0 * sp.pi,
                2 * sp.pi)
            dlmo_times = np.array(
            ...
            dlmo_times = dlmo_times[np.isfinite(dlmo_times)]
            dayYvalsDLMO = self.num_days - \
                np.arange(1.5, len(dlmo_times) + 1.5, 1.0)

        if (tsdf2.Phase.iloc[0] < sp.pi):
            cbt_phases = np.arange(sp.pi, real_days * 2.0 * sp.pi, 2 * sp.pi)
            cbt_times = np.array(
                list([fmod(x, 24.0) for x in list(map(dlmo_func, cbt_phases))]))
            cbt_times = cbt_times[np.isfinite(cbt_times)]
            dayYvalsCBT = self.num_days - \
                (np.arange(0.5, len(cbt_times) + 0.5, 1.0))
        else:
            cbt_phases = np.arange(
                sp.pi + 2 * sp.pi,
                real_days * 2.0 * sp.pi,
                2 * sp.pi)
            cbt_times = np.array(
            ...
            cbt_times = cbt_times[np.isfinite(cbt_times)]
            dayYvalsCBT = self.num_days - \
                np.arange(1.5, len(cbt_times) + 1.5, 1.0)
    ...
    def addMarker(self, time, col='red'):
        ...
        yval = [self.num_days - int(time / 24.0) + 0.5,
                self.num_days - int(time / 24.0) + 0.5 - 1,
                self.num_days - int(time / 24.0) + 0.5 - 2]
