...
parser.add_argument('-w', '--write',
                    ...
                    action='store_true',
                    default=False,
                    help="Create a csv with the data included"
                    )
...
def generated_sleep(ml_model_path, steps, hr):
    ...
    wake_predicted = torch.sigmoid(ml_model(data)).squeeze(0).squeeze(0).detach().numpy()
...
if args.json:
    ...
    if args.sleepmodel:
        awObj.wake = generated_sleep(ml_model_path=args.sleepmodel, steps=steps, hr=hr)
...
if args.dash:
        spm = SinglePopModel({'tau' : args.period})
        sol_spm = spm.integrate_model(np.hstack(ts), args.multiplier*np.hstack(steps), ic)
        plot_dashboard(np.hstack(steps), sol_spm)
...
if args.cbt:
    spm2 = SinglePopModel({'tau' : args.period})
    cbt_runs = spm2.integrate_observer(ts, args.multiplier*steps, ic, observer=SinglePopModel.CBTObs)
    acto.plot_phasemarker(
